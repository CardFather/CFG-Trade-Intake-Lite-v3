...
export async function setCustomerCreditCents(customerId:number|string, cents:number){
  ...
  if(existing){
    return (await shopifyAdmin(`/metafields/${existing.id}.json`,{
      method:"PUT",
      body: JSON.stringify({ metafield: { id: existing.id, value: String(cents), type: "number_integer" } })
    })).metafield;
  } else {
    return (await shopifyAdmin(`/metafields.json`,{
      method:"POST",
      body: JSON.stringify({ metafield: { namespace: NS, key: KEY, type: "number_integer", value: String(cents), owner_resource:"customer", owner_id: customerId } })
    })).metafield;
  }
}
