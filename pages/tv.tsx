...
// Make `big` available to all components in this file
...
function Row({ title, items, color }: { title: string; items: any[]; color: string }) {
  return (
    <div style={{ marginBottom: 18 }}>
      <div style={{ fontSize: 18, opacity: 0.85, marginBottom: 8 }}>{title}</div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(140px,1fr))", gap: 10 }}>
        {items.map((t: any) => (
          <div key={t.id} style={{ background: "#111", border: `2px solid ${color}`, borderRadius: 12, padding: "16px 12px", textAlign: "center" }}>
            ...
            <div style={{ fontSize: 12, opacity: 0.7 }}>{t.customer_name || t.customer_phone || "Guest"}</div>
          </div>
        ))}
        {!items.length && <div style={{ opacity: 0.5 }}>— none —</div>}
      </div>
    </div>
  );
}
