...
export default function Search(){
  ...
  const submit=(e:any)=>{
    ...
    r.push(`/search?q=${encodeURIComponent(local.trim())}`);
  };
...
function Results({results}:{results:any[]}){
  return (
    <div style={{display:"grid", gap:8}}>
      {results.map((t:any)=>(
        <div key={t.id} style={{border:"1px solid #333", borderRadius:10, padding:"10px 12px", display:"flex", justifyContent:"space-between"}}>
          <div>
